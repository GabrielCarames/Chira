...
const DisplayMessages = (({message, user, goToMessage, scrollToBottom}) => {

    const displayMessage = (message) => {
        if(message.message === 'false') {
            const img = new Image();
            img.src = message.image.title;
            return <img className="messages__image" loading='lazy' onLoad={() => scrollToBottom()} src={'http://localhost:3001/public/uploads/' + message.image.title} alt="contactImage" />
